...
import 'region_detail_screen.dart' show ApiSetupScreen;
          ...
          child: Column(
            ...
            children: [
              ...
              _Tile(
                ...
                title: 'Data Sources',
                subtitle: 'NASA GIBS, NOAA, IPCC AR6',
                onTap: () {},
              ),
              _Tile(
                icon: Icons.folder_outlined,
                title: 'KML Cache',
                subtitle: 'Manage offline layers',
                onTap: () {},
              ),
              const SizedBox(height: 20),
...
// ═══════════════════════════════════════════════════════════════════════════════
// LG CONNECT SCREEN — Manual IP + QR Scanner (both options)
// ═══════════════════════════════════════════════════════════════════════════════
...
class _LGConnectScreenState extends State<LGConnectScreen>
  ...
  final _portCtrl = TextEditingController(text: '2222');
  ...
  Future<void> _connect() async {
    ...
    try {
      ...
      _showError(e.toString()); debugPrint('[Connect] $e');
    } catch (e) {
      // Show the raw error — useful for debugging on real rig
      _showError('Failed: $e');
    } finally {
  ...
  /// Called when QR scanner decodes a result.
  /// Expected QR format: "lg://192.168.x.x:2222"  or plain "192.168.x.x"
  void _onQrScanned(String raw) {
    ...
    if (ip.contains(':')) {
      ...
      ip = parts[0];
      port = int.tryParse(parts[1]) ?? 2222;
    }

    _ipCtrl.text = ip;
    _portCtrl.text = port.toString();

    // Switch to manual tab so user can confirm before connecting
    _tabs.animateTo(0);

    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('QR scanned: $ip — tap Connect to proceed'),
...
class _QrTabState extends State<_QrTab> {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          // TODO(M2): Replace with mobile_scanner widget:
          //   MobileScanner(onDetect: (capture) {
          //     final barcode = capture.barcodes.first;
          //     if (barcode.rawValue != null) widget.onScanned(barcode.rawValue!);
          //   })
          //
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                _step('4', 'The IP and port fill in automatically — tap Connect'),
              ],
            ),
          ),
          const SizedBox(height: 24),
          ...
          ElevatedButton.icon(
            onPressed: () {
              ...
              // TODO(M2): start / stop MobileScanner controller
              if (!_scanning) return;
              // Simulate a scan result for dev testing:
              Future.delayed(const Duration(seconds: 3), () {
                if (mounted && _scanning) {
                  setState(() => _scanning = false);
                  widget.onScanned('lg://192.168.29.124:2222');
                }
              });
            },
            icon: Icon(
  ...
  // Corner bracket decorations for QR viewfinder
...
class _Corner extends StatelessWidget {
  ...
  const _Corner({
    required this.size, required this.thick, required this.r,
    ...
    return SizedBox(
      ...
      child: CustomPaint(
        painter: _CornerPainter(
            thick: thick, color: color, r: r, top: top, left: left),
      ),
    );
  }
}
