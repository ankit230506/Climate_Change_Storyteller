...
/// FEATURE: KML Cache Management Screen
///
/// PURPOSE:
/// Pre-downloads all 18 KML files (6 regions × 3 eras)
/// so the app works fully offline on demo day.
/// Shows cache size, download progress, and clear option.
///
/// WHY OFFLINE CACHE:
/// On demo day (museum/exhibition) internet may be unreliable.
/// Pre-downloading ensures smooth LG rig demos without
/// waiting for NASA GIBS API calls.
...
class _KmlCacheScreenState extends State<KmlCacheScreen> {
  ...
  // Download state per KML file
  final Map<String, _KmlStatus> _statusMap = {};
  bool   _isDownloadingAll = false;
  int    _cacheSize        = 0; // bytes
  int    _cachedCount      = 0;

  static const _totalFiles = 18; // 6 regions × 3 eras
  ...
  Future<void> _downloadAll() async {
    ...
    for (final region in kDefaultRegions) {
      for (final era in ClimateEra.values) {
        ...
        // Small delay to not overwhelm NASA GIBS API
                    ...
                    return Container(
                      ...
                      decoration: BoxDecoration(
                        color: status == _KmlStatus.cached
                            ? AppColors.good.withOpacity(0.05)
                            : AppColors.bg2,
